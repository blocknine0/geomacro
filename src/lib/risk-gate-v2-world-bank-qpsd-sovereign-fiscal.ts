...
/**
 * Shadow-only QPSD sovereign-fiscal module state.
 *
 * General-government and central-government debt are intentionally separate
 * source-specific peer universes. This builder never combines their raw values,
 * never pools QPSD with WDI/Eurostat, and does not activate a production
 * fallback by itself. Production wiring requires an explicit later promotion
 * backed by source-registry state, shadow/census evidence and the existing
 * fail-closed paid-deliverability gates.
 */
