...
import { loadStructuralContext, type StructuralContext, type StructuralObservation } from "./structural-context.server";
...
// The adaptive paid route must provide explicit checkers for them before it may
// advertise a query containing these modules as payable.
...
function moduleMatchesDimension(module: string, dimension: string) {
  ...
  return (STRUCTURAL_MODULE_ALIASES[module] ?? [module]).some((alias) => normalized === alias || normalized.includes(alias));
}

function structuralModulesFor(context: StructuralContext) {
  const modules = new Set<string>();
  for (const module of Object.keys(STRUCTURAL_MODULE_ALIASES)) {
    if (context.observations.some((row) => moduleMatchesDimension(module, row.dimension))) modules.add(module);
    if (context.metadata.coverage.some((row) => moduleMatchesDimension(module, row.dimension) && row.coverage_status.toUpperCase() !== "UNAVAILABLE")) modules.add(module);
  }
  return modules;
}
...
export async function checkAgentQueryDeliverability(
    ...
    if (context.status !== "AVAILABLE") {
      ...
      for (const module of requiredStructural) {
        ...
        if (latest !== null) {
          ...
          // No timestamp means freshness cannot be proven, so a paid query must fail closed.
          stale.add(module);
        }
      }
    }
  ...
  const deliverable = missingModules.length === 0 && staleModules.length === 0;
