...
export function applyGlobalSecurityHeaders(
  ...
  if (url.protocol === "https:") {
    // Do not includeSubDomains/preload until every current/future subdomain is
    // explicitly reviewed for HTTPS-only compatibility.
    headers.set("Strict-Transport-Security", "max-age=31536000");
  } else {
    headers.delete("Strict-Transport-Security");
  }

  // Application-layer fingerprint reduction only. Edge providers may still
  // add infrastructure headers outside application control.
...
export function secureServerResponse(
  ...
  // Preserve protocol-switching responses if the runtime supplies one.
