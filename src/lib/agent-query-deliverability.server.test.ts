...
describe("adaptive query pre-payment deliverability", () => {
  it("allows a fresh fully-covered structural query", async () => {
    ...
    const result = await checkAgentQueryDeliverability(plan, { now: new Date("2026-09-16T00:00:00.000Z") });
    expect(result.deliverable).toBe(true);
    expect(result.code).toBe("AVAILABLE");
  });

  it("fails closed before payment when a required module is absent", async () => {
    const onlyMacro = context({ observations: [context().observations[0]] });
    loadStructuralContext.mockResolvedValue(onlyMacro);
    const plan = buildAgentQueryPlan({ subjects: [{ type: "country", country_iso3: "USA" }], topics: ["macro_risk"], max_age_seconds: 172800 });
    const result = await checkAgentQueryDeliverability(plan, { now: new Date("2026-09-16T00:00:00.000Z") });
  ...
  it("requires explicit governed checker for Risk Gate/Risk Object/GRI modules", async () => {
    ...
    const denied = await checkAgentQueryDeliverability(plan, { now: new Date("2026-09-16T00:00:00.000Z") });
