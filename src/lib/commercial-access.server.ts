...
function bearerToken(request: Request): string {
  ...
  if (token.length < 24 || token.length > 512) {
    throw new CommercialAccessError(401, "COMMERCIAL_API_KEY_REQUIRED", "A valid Geomacro commercial API bearer key is required.");
  }
...
export async function authenticateCommercialApiRequest(
  ...
  if (!row?.ok) {
    throw new CommercialAccessError(401, String(row?.code ?? "COMMERCIAL_API_KEY_DENIED"), "The commercial API credential is not authorized.");
  }
...
export async function ensureCommercialCreditAccount(input: {
  ...
  if (error) {
    throw new CommercialAccessError(503, "COMMERCIAL_CREDIT_ACCOUNT_UNAVAILABLE", "Commercial credit account could not be resolved.");
  }
...
export async function consumeCommercialCapability(input: {
  ...
  if (!tierAllowsCapability(input.tier, input.capability)) {
    throw new CommercialAccessError(403, "CAPABILITY_NOT_INCLUDED", "The requested capability is not included in this Geomacro tier.");
  }
  ...
  if (error) {
    throw new CommercialAccessError(503, "COMMERCIAL_USAGE_UNAVAILABLE", "Commercial usage accounting is temporarily unavailable.");
  }

  const result = (data ?? {}) as CommercialCreditResult;
  if (!result.ok) {
    if (result.code === "INSUFFICIENT_CREDITS") {
      throw new CommercialAccessError(402, "INSUFFICIENT_CREDITS", "This account does not have enough Geomacro credits for the requested capability.");
    }
    throw new CommercialAccessError(403, String(result.code ?? "COMMERCIAL_ACCESS_DENIED"), "Commercial access could not be granted.");
  }
