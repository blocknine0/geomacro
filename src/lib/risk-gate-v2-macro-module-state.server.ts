...
export async function generateCountryRiskGateV2MacroModuleStates(
  ...
  try {
    ...
    // Migration 930 is the explicit QPSD production activation boundary. Before
    // that migration exists in a runtime, QPSD remains unavailable and the
    // older production hierarchy continues. Other QPSD/database failures remain
    // fail-closed and are not silently swallowed.
    if (
      !(
        error instanceof Error &&
