...
function driverForType(type: string | null): RiskGateV2Driver {
  // UCDP 1/2/3 are state-based, non-state and one-sided violence. These
  // existing Risk Gate driver labels are attribution buckets, not a claim that
  // every UCDP event is literally the named subtype.
...
/**
 * Build one global, release-consistent country snapshot from UCDP Candidate.
 *
 * Scoring is distributional rather than based on opaque fixed country weights:
 * each sovereign receives the larger of its event-count percentile and fatality
 * percentile over the same clean global release/window. Quiet countries can
 * receive zero observed organized-violence exposure only because the caller
 * supplies an explicit clean global release manifest.
 *
 * Coverage is PARTIAL because organized violence is only one part of the full
 * geopolitical-security ontology. UCDP Candidate is provisional; confidence is
 * capped below 1 and decays with source lag.
 */
export function buildRiskGateV2GeopoliticalSecuritySnapshot(input: {
  ...
  if (lagDays > RISK_GATE_V2_GEOPOLITICAL_MAX_SOURCE_LAG_DAYS) {
    return new Map();
  }
  ...
  const sovereigns = [...new Set(input.sovereign_iso3.map((iso3) => iso3.trim().toUpperCase()))]
    ...
    .sort();
  if (sovereigns.length === 0) throw new Error("At least one sovereign country is required");
  ...
  for (const iso3 of sovereigns) {
    ...
    states.set(iso3, {
      module_state_id: [
        ...
        input.release.manifest_hash.slice(0, 24),
