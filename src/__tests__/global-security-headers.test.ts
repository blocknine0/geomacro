...
describe("global server security headers", () => {
  it("applies low-risk browser controls without a restrictive source CSP", () => {
  ...
  it("adds HSTS only for HTTPS requests", () => {
    const https = applyGlobalSecurityHeaders(new Headers(), "https://geomacro.live/");
  ...
  it("preserves route-specific cache/content headers and removes app fingerprints", async () => {
