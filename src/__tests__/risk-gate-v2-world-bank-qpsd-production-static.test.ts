...
const migration = fs.readFileSync(
  "supabase/migrations/938_world_bank_qpsd_production_source.sql",
  "utf8",
);
...
const promotion = fs.readFileSync(
  ".github/workflows/qpsd-production-promotion-census.yml",
  "utf8",
);
...
describe("World Bank QPSD governed production sovereign-fiscal fallback", () => {
  ...
  it("requires a clean manifest, exact source state, provenance and >=20 peers", () => {
  ...
  it("tries QPSD only after WDI/Eurostat and before PPG", () => {
  ...
  it("ingests while scoring is locked, then promotes transactionally and proves 100+", () => {
    ...
    expect(promotion).toContain("execution_authorized_is_false == true");
    expect(promotion).toContain("risk-gate-v2-sovereign-fiscal-world-bank-qpsd-source-specific-");
    expect(promotion).toContain("if: ${{ failure() && steps.promote.outcome == 'success' }}");
    expect(promotion).toContain("enabled_for_commercial_signals: false");
  });
  ...
  it("does not alter execution authorization or real-money launch gates", () => {
    ...
    expect(promotion).not.toContain("COINBASE_X402_MAINNET_ACK");
  });
});
