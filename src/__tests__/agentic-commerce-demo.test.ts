...
describe("Agentic Commerce public demo contract", () => {
  ...
  it("keeps the free sandbox and paid path on the same fail-closed Risk Gate service", () => {
    ...
    expect(service).toContain('mode?: "PUBLIC_SANDBOX" | "X402_PAID"');
  ...
  it("does not silently promote structural evidence into GRI v1.2", () => {
    const structural = read("src/lib/structural-context.server.ts");
    const contract = read("src/lib/agentic-demo-contract.ts");
    const service = read("src/lib/agentic-demo-service.server.ts");

    expect(structural).toContain("commercial_structural_geopolitical_observations");
  ...
  it("keeps feedback persistence privacy-minimized", () => {
    const migration = read("supabase/migrations/034_agentic_demo_feedback.sql");
  ...
  it("matches the actual current Risk Gate decision contract", () => {
    const gatePage = read("src/routes/risk-gate.tsx");
    const gateContract = read("src/lib/risk-gate-contract.ts");

    expect(gateContract).toContain('| "CONTINUE"');
    expect(gateContract).toContain('| "REDUCE_LIMIT"');
    expect(gateContract).toContain('| "REQUIRE_APPROVAL"');
    expect(gateContract).toContain('| "PAUSE"');
    expect(gatePage).not.toContain('"REROUTE"');
  });
});
