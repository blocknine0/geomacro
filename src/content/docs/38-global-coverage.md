# 38. Global Coverage

Geomacro is designed as global geopolitical and macro risk intelligence, but different product layers have different coverage requirements.

## Public global layer

The public Geopolitical, Macroeconomic and Critical Minerals Risk Indices summarize qualifying evidence within their respective domains. Each domain is presented independently. If a domain has no current verified eligible evidence, it is not converted into a zero-risk reading.

The current public indices preserve the versioned GRI v1.2 parent methodology and verified proof lineage, but the historical combined GRI is no longer the public headline product.

## Subject-specific layer

Country and corridor Risk Objects are currently a **Private Pilot** product wedge. Coverage should be described only for subjects and inputs actually supported by the pilot data path. A dated controlled coverage census is evidence about that specific workflow and date, not a promise that every country, product or request shape is commercially deliverable.

## Historical layer

The separate historical-data infrastructure broadens reproducible source coverage across geopolitics, macro and strategic-resource datasets. Historical ingestion does not automatically make every dataset commercially eligible or suitable for live decision delivery.

"Global" therefore describes the intended intelligence scope, not a claim that every country, corridor, event family and data dimension has identical live coverage.
