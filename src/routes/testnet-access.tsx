    ...
    <main className="mx-auto max-w-6xl px-4 py-8 sm:px-6 sm:py-12" data-testnet-auth-flow={AUTH_FLOW}>
      ...
      <section className="mt-6 rounded-2xl border border-border/70 bg-card/30 p-6 sm:p-8">
        ...
        <div className="mt-5 grid gap-3 md:grid-cols-2 lg:grid-cols-4">
          <article className="rounded-xl border border-border/70 bg-background/40 p-4"><p className="font-mono text-[9px] uppercase tracking-[0.14em] text-muted-foreground">PEOPLE</p><h3 className="mt-2 text-base font-semibold">Research & decisions</h3><p className="mt-2 text-sm leading-6 text-muted-foreground">Ask a risk question, understand change drivers, and use GRI context.</p><p className="mt-3 rounded-lg bg-background p-3 text-xs text-muted-foreground">“What changed in geopolitical risk?”</p></article>
          <article className="rounded-xl border border-border/70 bg-background/40 p-4"><p className="font-mono text-[9px] uppercase tracking-[0.14em] text-muted-foreground">PRODUCTS</p><h3 className="mt-2 text-base font-semibold">Embed structured risk</h3><p className="mt-2 text-sm leading-6 text-muted-foreground">Country, corridor, GRI and signed Risk Object outputs for your workflows.</p><p className="mt-3 rounded-lg bg-background p-3 text-xs text-muted-foreground">IND → SGP → corridor intelligence</p></article>
          <article className="rounded-xl border border-border/70 bg-background/40 p-4"><p className="font-mono text-[9px] uppercase tracking-[0.14em] text-muted-foreground">AI AGENTS</p><h3 className="mt-2 text-base font-semibold">Governed decision context</h3><p className="mt-2 text-sm leading-6 text-muted-foreground">Use <code>risk_gate_bundle</code> when an agent needs risk reasons plus verification.</p><p className="mt-3 rounded-lg bg-background p-3 text-xs text-muted-foreground"><code>execution_authorized=false</code></p></article>
          <article className="rounded-xl border border-border/70 bg-background/40 p-4"><p className="font-mono text-[9px] uppercase tracking-[0.14em] text-muted-foreground">MACHINES</p><h3 className="mt-2 text-base font-semibold">Discover the contract</h3><p className="mt-2 text-sm leading-6 text-muted-foreground">Start from the public manifest, then authenticate and request structured output.</p><p className="mt-3 rounded-lg bg-background p-3 text-xs text-muted-foreground"><code>GET /api/testnet/manifest</code></p></article>
        </div>
      </section>

      <section id="x402-demo" className="mt-6 rounded-2xl border border-border/70 bg-card/30 p-6 sm:p-8">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div><p className="font-mono text-[10px] uppercase tracking-[0.16em] text-primary">X402 IN ACTION</p><h2 className="mt-2 text-2xl font-semibold">API call → 402 response → pay → retry → response</h2><p className="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">A visual walkthrough of the request-bound payment loop. This is a demo, not a real payment.</p></div>
          <span className="rounded-full border border-primary/30 bg-primary/5 px-3 py-1 font-mono text-[9px] uppercase tracking-[0.12em] text-primary">INTERACTIVE DEMO</span>
        </div>
        <div className="mt-6 grid gap-4 lg:grid-cols-[0.7fr_1.3fr]">
          <div className="grid gap-2 sm:grid-cols-5 lg:grid-cols-1">
            {[["01","API call"],["02","402 response"],["03","Pay"],["04","Retry"],["05","Response"]].map(([number,label], index) => (
              <button key={number} type="button" onClick={() => setDemoStep(index)} className={demoStep === index ? "rounded-xl border border-primary bg-primary/5 px-4 py-3 text-left" : "rounded-xl border border-border/70 bg-background/30 px-4 py-3 text-left"}>
                <span className="font-mono text-[9px] text-muted-foreground">{number}</span><span className="ml-3 text-sm font-medium">{label}</span>
              </button>
            ))}
          </div>
          <div className="rounded-xl border border-border/70 bg-background/70 p-4 sm:p-5">
            {demoStep === 0 ? <><p className="font-mono text-[9px] uppercase tracking-[0.14em] text-muted-foreground">REQUEST</p><pre className="mt-3 overflow-x-auto rounded-lg bg-background p-4 text-[11px] leading-5 text-muted-foreground">{'POST /api/testnet/intelligence\n\nrequest_id: country-demo-0001\ncapability: structural_country_digest\nsubject: country / IND'}</pre></>
              : demoStep === 1 ? <><p className="font-mono text-[9px] uppercase tracking-[0.14em] text-muted-foreground">HTTP 402</p><pre className="mt-3 overflow-x-auto rounded-lg bg-background p-4 text-[11px] leading-5 text-muted-foreground">{'payment.status: required\npayment.amount_due_usdc: ' + DEMO_PRICE + '\npayment.request_id: country-demo-0001\nreceiver: configured Testnet receiver'}</pre></>
              : demoStep === 2 ? <><p className="font-mono text-[9px] uppercase tracking-[0.14em] text-muted-foreground">PAYMENT</p><pre className="mt-3 overflow-x-auto rounded-lg bg-background p-4 text-[11px] leading-5 text-muted-foreground">{'Testnet USDC payment\nchain: supported Testnet\namount: API quote\ntx: recorded by paying wallet'}</pre></>
              : demoStep === 3 ? <><p className="font-mono text-[9px] uppercase tracking-[0.14em] text-muted-foreground">RETRY</p><pre className="mt-3 overflow-x-auto rounded-lg bg-background p-4 text-[11px] leading-5 text-muted-foreground">{'POST /api/testnet/intelligence\nrequest_id: country-demo-0001\npayment: chain_key + tx_hash + payer_address'}</pre></>
              : <><p className="font-mono text-[9px] uppercase tracking-[0.14em] text-muted-foreground">MACHINE RESPONSE</p><pre className="mt-3 overflow-x-auto rounded-lg bg-background p-4 text-[11px] leading-5 text-muted-foreground">{'status: settled\ncapability: structural_country_digest\ndelivery: structured\nexecution_authorized: false'}</pre></>}
      ...
      <section id="wallet-account" className="mt-6 rounded-2xl border border-border/70 bg-card/30 p-6 sm:p-8">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div><p className="font-mono text-[10px] uppercase tracking-[0.16em] text-primary">GET TESTNET ACCESS</p><h2 className="mt-2 text-2xl font-semibold">Test it in the public console.</h2><p className="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">Connect a wallet and sign the one-time access message. No transaction is created by sign-in.</p></div>
          {walletConnected ? <button type="button" onClick={() => void disconnectWallet()} disabled={busy} className="rounded-lg border border-border px-3 py-2 text-xs font-medium disabled:opacity-50">Disconnect wallet</button> : null}
        </div>
        <div className="mt-5 rounded-xl border border-border/70 bg-background/40 p-5">
          {loadingAccount ? <p className="text-sm text-muted-foreground">Checking Testnet access…</p> : active ? (
            <div className="flex flex-wrap items-center justify-between gap-4"><div><p className="text-sm font-semibold">{accountSummary}</p><p className="mt-1 text-xs text-muted-foreground">{displayName || "Wallet tester"} · {shortAddress(connectedWallet)}</p></div><a href="/testnet-console" className="rounded-lg bg-primary px-4 py-2.5 text-sm font-semibold text-primary-foreground">Open Public Testnet Console</a></div>
          ) : (
          ...
          <div className="mt-6 border-t border-border/70 pt-6">
            {!active ? <p className="rounded-xl border border-border/70 bg-background/40 p-4 text-sm text-muted-foreground">Sign in with a wallet above to create or manage developer credentials.</p> : (
              <>
                ...
                <div className="mt-6"><p className="font-medium">Your developer API keys</p><p className="mt-1 text-xs text-muted-foreground">API Keys stay visible. API Secrets are never returned again. Rotate replaces a credential and issues a new one-time secret.</p>
                  {developerKeys.length === 0 ? <p className="mt-3 rounded-lg border border-border/70 bg-background/40 p-4 text-sm text-muted-foreground">No developer keys created yet.</p> : <div className="mt-3 space-y-3">{developerKeys.map((key) => <div key={key.credential_id} className="rounded-xl border border-border/70 bg-background/40 p-4"><div className="flex flex-wrap items-start justify-between gap-3"><div><div className="flex flex-wrap items-center gap-2"><p className="font-medium">{key.label}</p><span className="rounded-full border border-border/70 px-2 py-0.5 text-[10px] text-muted-foreground">{integrationLabel(key.integration_type)}</span><span className="rounded-full border border-border/70 px-2 py-0.5 text-[10px] text-muted-foreground">{key.enabled && !key.revoked_at ? "Active" : "Revoked"}</span></div><code className="mt-2 block break-all text-xs text-muted-foreground">API Key: {key.key_id || "Unavailable"}</code><p className="mt-1 text-xs text-muted-foreground">Secret: hidden permanently after creation</p>{key.scopes?.length ? <p className="mt-1 break-words text-xs text-muted-foreground">Scopes: {key.scopes.join(", ")}</p> : null}</div><div className="flex flex-wrap gap-2">{key.key_id ? <button type="button" onClick={() => void copyText(key.key_id || "", "API Key copied.")} className="rounded-lg border border-border px-3 py-2 text-xs">Copy API Key</button> : null}{key.enabled && !key.revoked_at ? <><button type="button" onClick={() => void rotateCredential(key.credential_id)} disabled={busy} className="rounded-lg border border-border px-3 py-2 text-xs disabled:opacity-50">Rotate</button><button type="button" onClick={() => void revokeCredential(key.credential_id)} disabled={busy} className="rounded-lg border border-border px-3 py-2 text-xs text-destructive disabled:opacity-50">Revoke</button></> : null}</div></div></div>)}</div>}
                </div>
              </>
            )}
          </div>
      ...
      <section className="mt-6 rounded-2xl border border-border/70 bg-card/30 p-6">
        <details>
          <summary className="cursor-pointer list-none"><p className="font-mono text-[10px] uppercase tracking-[0.16em] text-primary">INTEGRATION DETAILS</p><h2 className="mt-2 text-2xl font-semibold">For builders who want the contract.</h2><p className="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">Discover → authenticate → request → 402 → pay → retry.</p></summary>
          <div className="mt-6 border-t border-border/70 pt-6">
            ...
            <div className="mt-4 grid gap-4 lg:grid-cols-3">
              <article className="rounded-xl border border-border/70 bg-background/40 p-4"><p className="font-semibold">Discover</p><p className="mt-2 text-xs leading-5 text-muted-foreground">Read the current machine contract.</p><pre className="mt-3 overflow-x-auto rounded-lg bg-background p-3 text-[10px] leading-5 text-muted-foreground">GET /api/testnet/manifest</pre></article>
              <article className="rounded-xl border border-border/70 bg-background/40 p-4"><p className="font-semibold">Authenticate</p><p className="mt-2 text-xs leading-5 text-muted-foreground">Keep the API Secret server-side.</p><pre className="mt-3 overflow-x-auto rounded-lg bg-background p-3 text-[10px] leading-5 text-muted-foreground">Authorization: GeomacroTest &lt;API_KEY&gt;.&lt;API_SECRET&gt;</pre></article>
              <article className="rounded-xl border border-border/70 bg-background/40 p-4"><p className="font-semibold">Request</p><p className="mt-2 text-xs leading-5 text-muted-foreground">Start with a country digest, then move to GRI, signed Risk Objects or Risk Gate.</p><pre className="mt-3 overflow-x-auto rounded-lg bg-background p-3 text-[10px] leading-5 text-muted-foreground">{'POST /api/testnet/intelligence\nrequest_id: country-demo-0001\ncapability: structural_country_digest\nsubject: country / IND'}</pre></article>
            </div>
            <div className="mt-4 grid gap-4 lg:grid-cols-2">
              <article className="rounded-xl border border-border/70 bg-background/40 p-4"><p className="font-semibold">AI agent pattern</p><pre className="mt-3 overflow-x-auto rounded-lg bg-background p-4 text-[10px] leading-5 text-muted-foreground">{'risk_gate_bundle\n→ receive 402\n→ pay quoted Testnet USDC\n→ retry same request_id\n→ verify machine response'}</pre></article>
              <article className="rounded-xl border border-border/70 bg-background/40 p-4"><p className="font-semibold">Account inspection</p><p className="mt-2 text-xs leading-5 text-muted-foreground">Inspect entitlement, usage, prices and payment configuration without consuming credits.</p><pre className="mt-3 overflow-x-auto rounded-lg bg-background p-4 text-[10px] leading-5 text-muted-foreground">GET /api/testnet/account</pre></article>
            </div>
      ...
      <section className="mt-6 rounded-2xl border border-primary/30 bg-primary/5 p-6 sm:p-8">
        <div className="flex flex-wrap items-end justify-between gap-4"><div className="max-w-2xl"><p className="font-mono text-[10px] uppercase tracking-[0.16em] text-primary">NEXT PRODUCTION MILESTONE</p><h2 className="mt-2 text-2xl font-semibold">Testnet is where you build. Mainnet is where production begins.</h2><p className="mt-2 text-sm leading-6 text-muted-foreground">Mainnet transaction features are not enabled here. This surface is for integration, verification and product discovery.</p></div><a href="/roadmap" className="rounded-lg border border-border bg-background/50 px-4 py-2.5 text-sm font-medium">View roadmap</a></div>
      </section>
      ...
      <section className="mt-6 rounded-2xl border border-border/70 bg-card/30 p-6">
        <details>
          <summary className="cursor-pointer list-none"><p className="font-mono text-[10px] uppercase tracking-[0.16em] text-primary">OPTIONAL FEEDBACK + X</p><h2 className="mt-2 text-2xl font-semibold">Share the test, or leave feedback.</h2><p className="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">Optional. It never blocks Testnet access.</p></summary>
          <div className="mt-5 border-t border-border/70 pt-5">
            <div className="flex flex-wrap gap-2">
              <a href={X_SHARE_URL} target="_blank" rel="noreferrer" onClick={() => setShareOpened(true)} className="rounded-lg bg-primary px-4 py-2.5 text-sm font-semibold text-primary-foreground">Share on X</a>
              <button type="button" onClick={openFollowIntent} disabled={followState === "confirmed"} className="rounded-lg border border-border px-4 py-2.5 text-sm font-medium disabled:opacity-70">{followState === "confirmed" ? "Followed ✓" : "Follow @GeomacroLive"}</button>
              <button type="button" onClick={() => setFeedbackOpen((value) => !value)} className="rounded-lg border border-border px-4 py-2.5 text-sm font-medium">{feedbackOpen ? "Hide feedback form" : "Give feedback (optional)"}</button>
            </div>
            ...
            {feedbackOpen ? <form onSubmit={submitFeedback} className="mt-5 grid gap-4 sm:grid-cols-2">
              ...
              <div className="sm:col-span-2"><label className="text-xs text-muted-foreground" htmlFor="feedbackMostValuable">Most valuable</label><textarea name="most_valuable" id="feedbackMostValuable" maxLength={1000} className="mt-2 min-h-24 w-full rounded-lg border border-border bg-background px-3 py-2.5 text-sm" /></div>
              <div className="sm:col-span-2"><label className="text-xs text-muted-foreground" htmlFor="feedbackFriction">Friction or confusion</label><textarea name="friction" id="feedbackFriction" maxLength={1000} className="mt-2 min-h-24 w-full rounded-lg border border-border bg-background px-3 py-2.5 text-sm" /></div>
              <div className="sm:col-span-2"><label className="text-xs text-muted-foreground" htmlFor="feedbackMissingCapability">Missing capability</label><textarea name="missing_capability" id="feedbackMissingCapability" maxLength={1000} className="mt-2 min-h-24 w-full rounded-lg border border-border bg-background px-3 py-2.5 text-sm" /></div>
