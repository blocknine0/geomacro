  ...
  server: {
    handlers: {
      ...
      POST: async ({ request }) => {
        ...
        // Validate that the requested demo resource is currently available before
        // asking the caller to pay. The result is intentionally not returned here.
        // This reduces the chance of charging a testnet agent for a subject that
        // would immediately fail closed because no compatible signed GRO exists.
        try {
          await runAgenticPreflightDemo(body, {
            mode: "X402_PAID",
        ...
        try {
          ...
          const result = await runAgenticPreflightDemo(body, {
            ...
            payment: {
              ...
              note:
                "Paid through Circle Gateway x402 on Arc Testnet. The settlement reference may be a Gateway batch reference rather than an immediate transaction hash.",
            },
          });
          ...
          const message = error instanceof Error ? error.message : "Circle x402 payment failed.";
          const paymentFailure =
            message.startsWith("PAYMENT_VERIFICATION_FAILED") ||
            message.startsWith("PAYMENT_SETTLEMENT_FAILED") ||
            message === "PAYMENT_SIGNATURE_MISSING";

          return json(
            {
              ok: false,
              error: {
                code: paymentFailure ? "X402_PAYMENT_FAILED" : "PAID_RESOURCE_FAILED_CLOSED",
                message,
              },
              payment: {
                provider: "circle_gateway_x402",
