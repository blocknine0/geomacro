...
const LEVELS: Array<{
...
}> = [
  {
    ...
    label: "Calm",
    max: 20,
    className: "text-risk-calm border-risk-calm/40 bg-risk-calm/10",
    description: "Global risk is subdued.",
  },
  {
    level: "stable",
    label: "Stable",
    max: 40,
    className: "text-risk-stable border-risk-stable/40 bg-risk-stable/10",
    description: "Global risk is stable.",
  },
  {
    level: "watch",
    label: "Watch",
    max: 60,
    className: "text-risk-watch border-risk-watch/40 bg-risk-watch/10",
    description: "Global risk is moderately elevated.",
  },
  {
    level: "elevated",
    label: "Elevated",
    max: 80,
    className: "text-risk-elevated border-risk-elevated/40 bg-risk-elevated/10",
    description: "Global risk is elevated.",
  },
  {
    level: "critical",
    label: "Critical",
    max: 101,
    className: "text-risk-critical border-risk-critical/40 bg-risk-critical/10",
    description: "Global risk is severe.",
  },
];
...
export function RiskBadge({
  ...
  return (
    <span
      className={cn(
        "inline-flex items-center gap-1.5 rounded-[var(--radius-control)] border px-2 py-0.5 type-meta",
        l.className,
        className,
      )}
    >
      {showScore && (
        <span className="type-metric text-[11px] tracking-normal">{Math.round(score)} / 100 ·</span>
      )}
      {l.label}
    </span>
  );
...
export function ProbabilityBadge({
  ...
  return (
    <span
      className={cn(
        "inline-flex items-center gap-1.5 rounded-[var(--radius-control)] border border-border/70 bg-card/60 px-2 py-0.5",
        className,
      )}
    >
