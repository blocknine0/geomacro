...
**PENDING REAL STAGING RUN.** The harness and production-host guard are CI-testable without secrets. A real HTTP load result must not be claimed until an isolated staging deployment, staging API client, environment variable, secret and explicit workflow-dispatch run exist.
