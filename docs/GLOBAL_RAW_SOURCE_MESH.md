...
The worker selects due country targets, fetches official web/API/global fallback sources concurrently, stores private raw snapshots, verifies storage by SHA-256 readback, creates chained normalized evidence fragments, directly invokes the canonical live structurer for new fragments, records target health, and fails the runtime coverage audit when recent successful raw acquisition is missing. Target reads are paginated beyond the platform's default row page, shared upstream hosts are paced, retryable rate-limit/network responses honor bounded backoff and Retry-After, and structural mesh-filler rows are excluded from live HTTP acquisition because runtime coverage requires only one successful target per country/category.
...
The runtime gate still requires a recent successful raw target in all three categories for every canonical country. The mesh filler rows preserve structural 3/4/6 topology only; they are not treated as independent upstream endpoints. No failure is hidden by changing freshness thresholds, disabling TLS verification, or treating an unreachable upstream source as a success.
