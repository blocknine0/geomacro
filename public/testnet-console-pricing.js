(() => {
  ...
  function ensureSelectContrast() {
    if (document.getElementById("geomacroNativeSelectContrast")) return;
  ...
  async function waitForCapabilitySelect(timeoutMs = 4000) {
  ...
  async function syncCanonicalPricing() {
    try {
      const response = await window.fetch("/api/testnet-tester/config", {
