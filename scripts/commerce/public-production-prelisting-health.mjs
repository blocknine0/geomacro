...
function adaptiveRequest() {
  const country = String(process.env.GEOMACRO_PRELISTING_COUNTRY_ISO3 ?? "USA")
    .trim()
    .toUpperCase();
  if (!/^[A-Z]{3}$/.test(country)) fail("GEOMACRO_PRELISTING_COUNTRY_ISO3 must be ISO3");
  return {
    ...
    client_request_id: `prelisting-public-health-${Date.now()}`,
  };
}
...
async function main() {
  ...
  const result = {
    schema_version: "geomacro.public-production-prelisting-health.v1",
  ...
  const output =
    process.env.GEOMACRO_PUBLIC_PRELISTING_HEALTH_OUTPUT?.trim() ||
    "artifacts/public-production-prelisting/prelisting-health.json";
  await mkdir(path.dirname(output), { recursive: true });
  await writeFile(output, JSON.stringify(result, null, 2) + "\n", { mode: 0o600 });
  console.log("PASS: exact-SHA public production pre-listing unpaid x402 health passed for Coinbase, Circle and Nevermined.");
