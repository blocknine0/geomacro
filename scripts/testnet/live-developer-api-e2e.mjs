...
async function provisionDeveloperCredential(cookie) {
  const existing = await jsonFetch(
    'developer keys',
    BASE_URL + '/api/testnet-tester/developer-keys',
    { headers: { cookie } },
  );
  const keys = Array.isArray(existing.payload?.data)
    ? existing.payload.data.filter((item) => item?.enabled === true && !item?.revoked_at)
    : [];
  ...
  if (keys.length === 0) {
    const issued = await jsonFetch(
      ...
      BASE_URL + '/api/testnet-tester/developer-key',
      {
        ...
        headers: { cookie },
        body: JSON.stringify({
          label: 'Geomacro automated E2E',
          integration_type: 'ai_agent',
        }),
      },
      [200],
    );
    assert(issued.payload?.ok === true, 'Developer credential issuance failed');
    const data = issued.payload?.data ?? {};
    return {
  ...
  const credentialId = String(keys[0].credential_id ?? '');
  assert(/^[0-9a-fA-F-]{36}$/.test(credentialId), 'Existing Developer credential id is invalid');

  const rotated = await jsonFetch(
    'developer key rotate',
  ...
  assert(rotated.payload?.ok === true, 'Developer credential rotation failed');
  const data = rotated.payload?.data ?? {};
  return {
    apiKey: String(data.api_key ?? ''),
    apiSecret: String(data.api_secret ?? ''),
    credentialId: String(data.credential_id ?? ''),
...
async function main() {
  ...
  const challenge = await jsonFetch(
    ...
    BASE_URL + '/api/testnet-tester/auth-challenge',
    {
      ...
      body: JSON.stringify({ wallet_address: wallet.address, chain_id: CHAINS[CHAIN_KEY] }),
    },
  );
  assert(challenge.payload?.ok === true, 'Wallet challenge failed');
  const challengeData = challenge.payload?.data;
  assert(challengeData?.message && challengeData?.nonce && challengeData?.issued_at, 'Wallet challenge is incomplete');
  ...
  const verified = await jsonFetch(
    ...
    BASE_URL + '/api/testnet-tester/auth-verify',
    {
      ...
      body: JSON.stringify({
        ...
        issued_at: challengeData.issued_at,
        message: challengeData.message,
        signature,
        profile_name: 'Geomacro Developer API E2E',
      }),
    },
  );
  assert(verified.payload?.ok === true && verified.payload?.data?.access_status === 'active', 'Wallet verification did not activate Testnet access');
  const COOKIE = extractCookie(verified.response.headers);
  assert(COOKIE.includes('='), 'Testnet session cookie is invalid');
  ...
  for (const capability of CAPABILITIES) {
    ...
    const quote = await jsonFetch('402 quote ' + capability, BASE_URL + '/api/testnet/intelligence', { method: 'POST', headers: authHeaders(API_KEY, API_SECRET), body: JSON.stringify(request) }, [402]);
  ...
  const settled = await jsonFetch('developer paid delivery', BASE_URL + '/api/testnet/intelligence', { method: 'POST', headers: authHeaders(API_KEY, API_SECRET), body: JSON.stringify({ ...paidSeed.request, payment: proof }), timeout_ms: 30000 });
  ...
  const replay = await jsonFetch('developer exact replay', BASE_URL + '/api/testnet/intelligence', { method: 'POST', headers: authHeaders(API_KEY, API_SECRET), body: JSON.stringify({ ...paidSeed.request, payment: proof }) });
  ...
  const crossRequest = await jsonFetch('developer cross-request payment replay', BASE_URL + '/api/testnet/intelligence', { method: 'POST', headers: authHeaders(API_KEY, API_SECRET), body: JSON.stringify({ ...paidSeed.request, request_id: rid('cross-request'), payment: proof }) }, [409]);
  ...
  const changed = await jsonFetch('developer same-id changed-payload replay', BASE_URL + '/api/testnet/intelligence', { method: 'POST', headers: authHeaders(API_KEY, API_SECRET), body: JSON.stringify(changedPayload) }, [400, 409]);
  ...
  const report = {
    ...
    chain_key: CHAIN_KEY,
    developer_credential_action: credentials.action,
    developer_credential_id: credentials.credentialId,
    wallet_address: wallet.address,
