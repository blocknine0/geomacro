...
async function provisionDeveloperCredential(cookie) {
  ...
  const active = Array.isArray(existing.payload?.data)
    ? existing.payload.data.find((item) => item?.enabled === true && !item?.revoked_at)
    : null;
  ...
  const credentialId = String(active.credential_id ?? '');
...
async function main() {
  ...
  for (const capability of CAPABILITIES) {
    ...
    const quote = await jsonFetch('402 quote ' + capability, BASE_URL + '/api/testnet/intelligence', { method: 'POST', headers: authHeaders(apiKey, apiSecret), body: JSON.stringify(request) }, [402]);
  ...
  const settled = await jsonFetch('developer paid delivery', BASE_URL + '/api/testnet/intelligence', { method: 'POST', headers: authHeaders(apiKey, apiSecret), body: JSON.stringify({ ...paidSeed.request, payment: proof }), timeout_ms: 30000 });
  ...
  const replay = await jsonFetch('developer exact replay', BASE_URL + '/api/testnet/intelligence', { method: 'POST', headers: authHeaders(apiKey, apiSecret), body: JSON.stringify({ ...paidSeed.request, payment: proof }) });
  ...
  const crossRequest = await jsonFetch('developer cross-request payment replay', BASE_URL + '/api/testnet/intelligence', { method: 'POST', headers: authHeaders(apiKey, apiSecret), body: JSON.stringify({ ...paidSeed.request, request_id: rid('cross-request'), payment: proof }) }, [409]);
  ...
  const changed = await jsonFetch('developer same-id changed-payload replay', BASE_URL + '/api/testnet/intelligence', { method: 'POST', headers: authHeaders(apiKey, apiSecret), body: JSON.stringify(changedPayload) }, [400, 409]);
