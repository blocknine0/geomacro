-- Idempotent production convergence for the Early Warning market-impact extension.
-- Repairs environments where migration 939 was not fully applied, without
-- changing public table access, RLS, payment, settlement, or execution behavior.
...
begin
  if not exists (
    ...
    where conrelid = 'public.early_warning_alerts'::regclass
      and conname = 'early_warning_market_impact_object_check'
  ) then
    alter table public.early_warning_alerts
  ...
  if not exists (
    ...
    from pg_constraint
    where conrelid = 'public.early_warning_alerts'::regclass
      and conname = 'early_warning_market_impact_binding_check'
  ) then
    alter table public.early_warning_alerts
