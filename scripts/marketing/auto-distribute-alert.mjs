...
for (const field of ['alert_id', 'country', 'country_iso3', 'status', 'confidence', 'event_title', 'primary_cause', 'detected_at_utc', 'country_timezone', 'detected_at_local']) {
  requireField(field);
}
...
const allowedStatus = config.public_alert_policy.allowed_statuses.includes(status);
...
const eligible = allowedStatus && confidence >= config.public_alert_policy.minimum_confidence && evidenceOkay;

if (!eligible && !args.has('force')) {
  console.log(JSON.stringify({ published: false, reason: 'alert_not_publicly_eligible', status, confidence, evidenceCount }, null, 2));
  process.exit(0);
}
...
function detailedBase() {
  return [
    ...
    `UTC: ${alert.detected_at_utc}`,
    '',
    'This is structured risk intelligence, not a buy/sell signal.',
    url
...
function render(channel) {
  ...
  const text = channel === 'telegram' || channel === 'discord' ? detailedBase() : compactBase();
...
async function sendMastodon(text, idempotencyKey) {
  ...
  const response = await fetch(`${base}/api/v1/statuses`, {
    method: 'POST',
    headers: { authorization: `Bearer ${token}`, 'content-type': 'application/x-www-form-urlencoded', 'Idempotency-Key': idempotencyKey },
    body
  });
