...
import { canonicalJson, methodologyManifest } from './gri-engine-v12.js';
...
export function calculationManifest(calculation) {
  return {
    ...
    contributions: calculation.contributions.map((c) => ({
      ...
      storyStrongestSourceWeight: roundNumber(c.storyStrongestSourceWeight, 10),
...
export function evidenceManifest(calculation) {
  return calculation.contributions
    .map((c) => ({
      ...
      summary: c.summary,
      storyClusterId: c.storyClusterId,
      storyCanonicalLabel: c.storyCanonicalLabel,
      storyAssignmentDecision: c.storyAssignmentDecision,
      storyMatchConfidence: roundNumber(c.storyMatchConfidence, 6),
      storyDecisionRationale: c.storyDecisionRationale,
      storyClusteringProvider: c.storyClusteringProvider,
      storyClusteringModel: c.storyClusteringModel,
      storyClusteringVersion: c.storyClusteringVersion,
      storyClusteringPromptVersion: c.storyClusteringPromptVersion,
      storyClusteringScoredAt: c.storyClusteringScoredAt,
      storyClusteringInputHash: c.storyClusteringInputHash,
      severity: roundNumber(c.severity, 10),
      confidence: roundNumber(c.confidence, 10),
      observedAt: c.observedAt,
...
export function buildDeterministicExplanation(
  ...
  return {
    ...
    why: {
      ...
      storyCountDelta: attribution.storyCountDelta,
