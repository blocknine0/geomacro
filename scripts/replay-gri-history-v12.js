...
const replayVersion = 'gri-replay-v1.2.0';
...
async function attachCurrentStoryProvenance(events) {
  ...
  for (let i = 0; i < eventIds.length; i += batchSize) {
    ...
    for (const assignment of data ?? []) {
      if (assignmentsByEventId.has(assignment.event_id)) {
        throw new Error(
          `GRI v1.2 story provenance rejected: duplicate current-contract ` +
  ...
  if (missingAssignments.length > 0) {
    throw new Error(
      `GRI v1.2 calculation blocked: ${missingAssignments.length} canonical ` +
  ...
  const enriched = events.map((event) => {
    ...
    if (!cluster) {
      throw new Error(
        `GRI v1.2 story provenance rejected: cluster ` +
    ...
    if (
      ...
      throw new Error(
        `GRI v1.2 story/category mismatch for event ${event.id}: ` +
    ...
    if (
      ...
      throw new Error(
        `GRI v1.2 rejected non-current story cluster ${cluster.id}`
      );
    }
    ...
    if (!hasCanonicalStoryProvenance(row)) {
      throw new Error(
        `GRI v1.2 story provenance rejected for event ${event.id}`
      );
    }
  ...
  console.log(
    `GRI v1.2 story provenance: ${enriched.length} assigned event(s), ` +
...
async function main() {
  ...
  const { data: run, error: runError } = await supabase.from('gri_replay_runs').insert({
    ...
    notes: 'Retrospective calibration replay under gri-v1.2.0. Historical created_at is used as an observation-time proxy; current-contract classifications and immutable story assignments may have been generated later. lookahead_safe=false. Never present this run as historical live/OOS performance.',
  }).select('id').single();
