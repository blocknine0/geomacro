...
Deno.serve(async (request) => {
  ...
  const { data: events, error } = await db
    ...
    .select("flash_id,source_record_id,published_at,source_updated_at_utc,detection_latency_ms,headline,source_channel,source_url,event_type,signal_category,source_version,material_update,verification_status,severity_bps,source_reliability_bps,verification_score_bps,latitude_e6,longitude_e6,content_hash,created_at")
    .is("archived_at", null)
