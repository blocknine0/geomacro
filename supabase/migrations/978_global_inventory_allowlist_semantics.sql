...
--
-- EIA API v2 and NOAA NCEI CDO are intentionally governed ingestion-only
-- sources enabled by 975_enable_governed_source_ingestion.sql. The inventory
-- view must not treat these explicit governed sources as unexpected enablement.
--
-- This migration changes only the read-only inventory status view. It does
-- not enable ingestion, promote commercial rights, certify endpoints, or
-- certify delivery.
-- =============================================================================
...
new_source_enablement as (
  ...
  where (
    ...
    or source_id in (
      ...
      'iata_wis','eurocontrol_nod','copernicus_ems',
      'noaa_swpc','usgs_volcanoes','wmo_gts',
      'gdelt_v2_events','eia_api_v2','noaa_ncei_cdo_api'
    )
  )
...
comment on view public.live_global_source_inventory_100_status is
 'Global inventory gate including the category-specific Critical Minerals source universe. Inventory completeness does not certify endpoints, rights, schemas, freshness or commercial delivery. Governed ingestion-only exceptions explicitly allow GDELT V2, EIA API v2 and NOAA NCEI CDO because they are enabled only for controlled ingestion and remain outside commercial signal certification.';
