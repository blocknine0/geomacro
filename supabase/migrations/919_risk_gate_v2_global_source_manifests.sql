...
create table if not exists public.live_source_release_manifests (
  ...
  verified_rows integer not null default 0 check (verified_rows >= 0),
  partial_rows integer not null default 0 check (partial_rows >= 0),
...
alter table public.live_source_release_manifests enable row level security;
revoke all on public.live_source_release_manifests from public, anon, authenticated;
grant select, insert, update on public.live_source_release_manifests to service_role;

create index if not exists live_source_release_manifests_latest_idx
on public.live_source_release_manifests (source_id, coverage_end desc, retrieved_at desc)
...
values (
  ...
  'GLOBAL',
  'ANNUAL',
  'WGI 2025 Revision political-stability bundle only. Derived Risk Gate use; do not generalize this state to underlying third-party WGI source material.'
)
...
comment on table public.live_source_release_manifests is
  'Release-level completeness evidence for governed global sources. A clean manifest can support evidence-backed no-observed-event states; absence of a manifest cannot.';
