...
begin
  ...
  if p_expected_response_sha256 !~ '^[0-9a-f]{64}
  select *
    into v_payment
  ...
  if coalesce(v_delivery_response_payload ->> 'delivered_product_hash', '') <> p_expected_delivered_product_hash then
...
comment on function public.reconcile_agent_commerce_payment(uuid,text,text,text,text,boolean) is
 ...
 then
  ...
  if p_expected_delivered_product_hash !~ '^[0-9a-f]{64}
  select *
    into v_payment
  from public.commercial_payment_events p
  where p.id = p_payment_event_id
