...
--
-- PURPOSE
-- - promote a settled production payment to a matched accounting state only
--   when provider settlement, durable delivery and successful usage evidence agree
-- - keep controlled internal canaries explicitly non-revenue
-- - permit commercial revenue only for external_customer or
--   independent_production_buyer purchases
-- - support Coinbase's dedicated delivery ledger plus the provider-neutral
--   Circle/Nevermined ledger without weakening replay boundaries
-- =============================================================================

-- Coinbase predates the provider-neutral ledger. Preserve that state machine and
-- add the exact prepared-response hash required for independent reconciliation.
...
begin
  ...
  if p_expected_delivered_product_hash !~ '^[0-9a-f]{64}$' then
    raise exception 'expected delivered product hash is invalid';
  end if;

  if p_internal_canary is null then
    raise exception 'reconciliation must explicitly identify internal canary vs commercial purchase';
  end if;

  if p_purchase_classification not in (
    'internal_canary',
    'external_customer',
    'independent_production_buyer'
  ) then
    raise exception 'unsupported purchase classification';
  end if;

  if p_internal_canary is true
     and p_purchase_classification <> 'internal_canary' then
    raise exception 'internal canary flag/classification mismatch';
  end if;

  if p_internal_canary is false
     and p_purchase_classification not in (
       'external_customer',
       'independent_production_buyer'
     ) then
    raise exception 'commercial revenue requires an external or independent production buyer classification';
  end if;
  ...
  if v_payment.environment not in ('mainnet','fiat') then
    raise exception 'only mainnet or fiat payment events can be reconciled here';
  end if;
  ...
  if trim(coalesce(v_payment.provider_settlement_id, ''))
     <> trim(p_expected_provider_settlement_id) then
    raise exception 'provider settlement id does not match expected reconciliation evidence';
  end if;

  -- Coinbase keeps its original dedicated ledger; Circle and Nevermined use the
  ...
  if coalesce(v_delivery_response_payload ->> 'delivered_product_hash', '')
     <> p_expected_delivered_product_hash then
    raise exception 'buyer-observed delivered product hash does not match durable delivery payload';
  end if;

  if not (v_delivery_response_payload ? 'execution_authorized')
     or (v_delivery_response_payload ->> 'execution_authorized')::boolean is distinct from false then
    raise exception 'execution boundary violated or missing in delivered response';
  end if;

  if trim(coalesce(v_delivery_settlement_reference, ''))
     <> trim(v_payment.provider_settlement_id) then
  ...
  v_network_ok :=
    ...
    or (
      v_payment.environment = 'fiat'
      and v_payment.network_family = 'offchain'
      and coalesce(v_payment.network_name, '') = coalesce(v_delivery_settlement_network, '')
    );
  ...
  -- proven again and the stored classification matches the requested mode.
  if v_payment.reconciliation_status = 'matched'
     and coalesce(v_payment.metadata ->> 'purchase_classification', '')
         = p_purchase_classification
     and (
       (
    ...
    return query
      select
        ...
        v_payment.revenue_classification,
        v_payment.commercial_revenue,
        v_delivery_id,
        v_delivery_response_sha256;
    return;
  end if;
...
revoke all on function public.reconcile_agent_commerce_payment(
  uuid,text,text,text,text,boolean,text
) from PUBLIC, anon, authenticated;
grant execute on function public.reconcile_agent_commerce_payment(
  uuid,text,text,text,text,boolean,text
) to service_role;

-- These four accounting columns may not be directly promoted by application
-- service-role code. The SECURITY DEFINER reconciliation function above is the
-- only application path that may move these providers into matched revenue.
revoke update (
...
begin
  if new.provider in (
    ...
    if new.environment not in ('mainnet','fiat')
       ...
       or coalesce(new.metadata ->> 'reconciled_response_sha256', '') !~ '^[0-9a-f]{64}$'
       or coalesce(new.metadata ->> 'reconciled_delivered_product_hash', '') !~ '^[0-9a-f]{64}$'
       or coalesce((new.metadata ->> 'internal_canary')::boolean, true) is distinct from false
       or coalesce(new.metadata ->> 'purchase_classification', '')
          not in ('external_customer','independent_production_buyer') then
      raise exception 'agent-commerce revenue promotion requires matched external delivery evidence';
    end if;
  end if;
...
comment on function public.reconcile_agent_commerce_payment(
  uuid,text,text,text,text,boolean,text
) is
  'Reconciles settled Coinbase/Circle/Nevermined production payments against durable delivery, buyer-observed product identity and usage evidence. Internal canaries remain non-revenue; commercial revenue requires an external or independent production buyer.';
