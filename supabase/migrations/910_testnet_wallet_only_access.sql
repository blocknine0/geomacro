-- Geomacro Testnet tester wallet-only access v2.1
-- Registration identity is a verified EVM wallet. Email/X/Discord are not activation gates.
-- Supported payment networks are enforced by the application registry: Arc Testnet,
-- Base Sepolia and Polygon Amoy. Testnet settlement remains non-revenue.
...
begin
  ...
  if v_profile.registration_status <> 'complete'
    or v_profile.wallet_verified_at is null then
...
) is 'Activates one fixed 500-credit, 30-day Geomacro Testnet quota after wallet verification and a verified 0.50 Testnet USDC payment. Email/X/Discord are not registration gates. Testnet settlement is non-revenue.';
