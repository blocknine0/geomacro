...
--
-- Extends the service-role-only receipt ledger created in migration 937 with:
...
--
-- IMPORTANT
-- - this migration does not enable public distribution
-- - ambiguous external delivery outcomes are NOT automatically retryable
-- - only service_role may call the claim/finalize RPCs
-- =============================================================================
...
begin
  if not exists (
    ...
    alter table public.early_warning_distribution_receipts
      ...
      check (payload_hash is null or payload_hash ~ '^[0-9a-f]{64}create index if not exists early_warning_distribution_lease_idx
...
begin
  ...
  if p_response_code is not null and (p_response_code < 100 or p_response_code > 599) then
  ...
  set status = case p_outcome
      ...
      published_at = case when p_outcome = 'PUBLISHED' then v_now else null end,
