...
-- Extend the existing certification queue for direct monetary-authority paths.
...
alter table public.live_source_certification_queue
  ...
  check (
    source_role in (
      ...
      'GOVERNMENT_PORTAL', 'STATISTICS_OFFICE',
      'MONETARY_AUTHORITY'
    )
  );
...
select
 'COUNTRY:'||r.iso3||':MONETARY_AUTHORITY:monetary_'||lower(d.country_iso2),
 'COUNTRY',r.iso3,NULL,'MONETARY_AUTHORITY',
 'monetary_'||lower(d.country_iso2),
...
begin
  if new.enabled then
    ...
    select
      ...
      'COUNTRY',new.iso3,'MONETARY_AUTHORITY',
      'monetary_'||lower(d.country_iso2),6,true,
      'Auto-mapped national/shared monetary-authority source candidate.'
    from public.live_country_monetary_authority_directory d
    where upper(d.country_iso2)=upper(new.iso2)
    on conflict(scope_type,scope_code,source_role,source_id) do update set
      priority=excluded.priority,required=excluded.required,notes=excluded.notes,updated_at=now();
    ...
    select
      ...
      'monetary_'||lower(d.country_iso2),
      'Auto-queued new country monetary-authority source.'
    from public.live_country_monetary_authority_directory d
    where upper(d.country_iso2)=upper(new.iso2)
    on conflict(queue_key) do nothing;
  end if;
...
('woah_wahis','WOAH WAHIS Animal Health Information','World Organisation for Animal Health','MULTI_DOMAIN','HTML','NONE','https://wahis.woah.org/',null,'REVIEW_REQUIRED',false,true,false,false,'GLOBAL','NEAR_REAL_TIME','Official animal-disease event and monitoring information; public interface supports analysis and risk-based decision-making.'),
('iaea_news_events','IAEA News and Events','International Atomic Energy Agency','GEOPOLITICS','HTML','NONE','https://www.iaea.org/newscenter',null,'REVIEW_REQUIRED',false,true,false,false,'GLOBAL','DAILY','Official nuclear/radiological safety and geopolitical event source candidate.'),
('wmo_global_weather','WMO World Meteorological Organization','World Meteorological Organization','MULTI_DOMAIN','HTML','NONE','https://wmo.int/',null,'REVIEW_REQUIRED',false,true,false,false,'GLOBAL','NEAR_REAL_TIME','Official global meteorological, climate and early-warning source candidate.'),
('ocha_hdx_api','OCHA Humanitarian Data Exchange','United Nations OCHA','MULTI_DOMAIN','API','NONE','https://data.humdata.org/about/api',null,'REVIEW_REQUIRED',false,true,false,false,'GLOBAL','DAILY','Humanitarian data API and country/cluster data discovery; exact dataset rights remain dataset-specific.'),
('wfp_hungermap_live','WFP HungerMap LIVE','World Food Programme','MULTI_DOMAIN','HTML','NONE','https://hungermap.wfp.org/',null,'REVIEW_REQUIRED',false,true,false,false,'GLOBAL','NEAR_REAL_TIME','Food security/acute hunger monitoring candidate; exact machine extraction and reuse contract pending.'),
('entsoe_transparency','ENTSO-E Transparency Platform','ENTSO-E','MACRO','API','NONE','https://transparency.entsoe.eu/',null,'REVIEW_REQUIRED',false,true,false,false,'EUROPE','NEAR_REAL_TIME','European electricity system generation/load/interconnector data candidate.'),
('opec_data','OPEC Data','Organization of the Petroleum Exporting Countries','MACRO','HTML','NONE','https://www.opec.org/data.html',null,'REVIEW_REQUIRED',false,true,false,false,'GLOBAL','MONTHLY','Official oil production/market data and publications candidate.'),
('cloudflare_radar_outages','Cloudflare Radar Outages API','Cloudflare','MULTI_DOMAIN','API','API_TOKEN','https://developers.cloudflare.com/api/resources/radar/subresources/annotations/subresources/outages/','CC BY-NC 4.0','REVIEW_REQUIRED',false,true,false,false,'GLOBAL','NEAR_REAL_TIME','Internet outage/anomaly API. Non-commercial licence means paid commercial reuse remains explicitly blocked pending permission.'),
('ripe_ris_routing','RIPE NCC Routing Information Service API','RIPE NCC','MULTI_DOMAIN','API','NONE','https://stat.ripe.net/',null,'REVIEW_REQUIRED',false,true,false,false,'GLOBAL','NEAR_REAL_TIME','BGP routing visibility and Internet infrastructure source candidate; exact reuse boundary remains gated.'),
('imo_maritime_safety_information','IMO Maritime Safety Information','International Maritime Organization','GEOPOLITICS','HTML','NONE','https://www.imo.org/',null,'REVIEW_REQUIRED',false,true,false,false,'GLOBAL','NEAR_REAL_TIME','Maritime safety/navigation source candidate; machine interface and reuse remain certification-gated.'),
('ukmto_msi','UKMTO Maritime Security Information','UK Maritime Trade Operations','GEOPOLITICS','HTML','NONE','https://www.ukmto.org/',null,'REVIEW_REQUIRED',false,true,false,false,'GLOBAL','NEAR_REAL_TIME','Operational maritime security information candidate.'),
('marad_msci','MARAD Maritime Security Communications','U.S. Maritime Administration','GEOPOLITICS','HTML','NONE','https://www.maritime.dot.gov/msci-advisories',null,'REVIEW_REQUIRED',false,true,false,false,'GLOBAL','NEAR_REAL_TIME','Official maritime security advisory source.'),
('iata_wis','IATA World Air Transport Statistics','International Air Transport Association','MACRO','HTML','NONE','https://www.iata.org/en/publications/economics/statistics/','PERMISSION_REQUIRED','PERMISSION_REQUIRED',false,true,false,false,'GLOBAL','MONTHLY','Aviation traffic/economic dataset candidate; rights are not assumed for paid derived products.'),
('eurocontrol_nod','EUROCONTROL Network Operations Data','EUROCONTROL','MULTI_DOMAIN','HTML','NONE','https://www.eurocontrol.int/network-operations',null,'REVIEW_REQUIRED',false,true,false,false,'EUROPE','NEAR_REAL_TIME','European aviation/network operations source candidate.'),
('copernicus_ems','Copernicus Emergency Management Service','European Commission','MULTI_DOMAIN','HTML','NONE','https://emergency.copernicus.eu/',null,'REVIEW_REQUIRED',false,true,false,false,'GLOBAL','NEAR_REAL_TIME','Earth-observation emergency mapping/rapid mapping candidate; exact product licence is dataset-specific.'),
('noaa_swpc','NOAA Space Weather Prediction Center','NOAA','MULTI_DOMAIN','API','NONE','https://www.swpc.noaa.gov/',null,'REVIEW_REQUIRED',false,true,false,false,'GLOBAL','NEAR_REAL_TIME','Space-weather/solar-storm warning source candidate.'),
('usgs_volcanoes','USGS Volcano Hazards Program','USGS','MULTI_DOMAIN','HTML','NONE','https://www.usgs.gov/programs/VHP',null,'REVIEW_REQUIRED',false,true,false,false,'GLOBAL','NEAR_REAL_TIME','Volcanic activity and hazard source candidate.'),
('usgs_earthquake_hazards','USGS Earthquake Hazards Program feeds and catalog','U.S. Geological Survey','MULTI_DOMAIN','JSON','NONE','https://earthquake.usgs.gov/earthquakes/feed/','USGS-produced data: U.S. public domain; source credit requested','COMMERCIAL_OK',false,true,false,false,'GLOBAL','REAL_TIME_MINUTE','USGS GeoJSON summary feeds are updated every minute. USGS-produced data may be reused with source credit; third-party copyrighted material is excluded. Risk Gate climate_environment_hazard remains blocked until schema and methodology are versioned.'),
('wmo_gts','WMO Global Telecommunication System','World Meteorological Organization','MULTI_DOMAIN','MIXED','NONE','https://wmo.int/activities/wmo-information-system',null,'REVIEW_REQUIRED',false,true,false,false,'GLOBAL','NEAR_REAL_TIME','Global observation/data-exchange infrastructure candidate; machine access contract remains gated.')
on conflict(source_id) do update set
