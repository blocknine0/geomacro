-- =============================================================================
-- Geomacro global coverage inventory contract correction
--
-- Permanent contract alignment:
--   1. Country source-universe inventory includes the three Critical Minerals
--      country paths (RMIS, USGS and BGS) in addition to the 21 base paths.
--   2. Critical Minerals country-path completeness recognizes the actual
--      governed universe: one RMIS path for each RMIS country record plus
--      USGS and BGS paths for every enabled canonical area.
--
-- This migration changes only read-only coverage status views. It does not
-- certify sources, promote commercial rights, enable ingestion or alter data.
-- =============================================================================
...
comment on view public.live_global_source_inventory_100_status is
 'Global inventory gate including the category-specific Critical Minerals source universe. Inventory completeness does not certify endpoints, rights, schemas, freshness or commercial delivery. The country universe includes 21 base paths plus 3 Critical Minerals paths per enabled area, with RMIS represented only where the governed RMIS country directory exists.';
