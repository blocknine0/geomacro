...
<main>
  ...
  <section class="hero">
    ...
    <p>Create a tester profile, verify an EVM wallet with a non-transaction signature, activate one fixed 500-credit quota at 0.5 Testnet USDC per credit, then test Geomacro intelligence, developer API access and the professional X share-card flow.</p>
  ...
  <section class="grid">
    ...
    <div class="card"><strong>500 fixed test credits</strong><p>0.5 Testnet USDC per credit. The fixed 500-credit Testnet quota costs 250 Testnet USDC total for 30 days. Testnet settlement is non-revenue.</p></div>
    <div class="card"><strong>Developer API + share card</strong><p>Create an API Key + API Secret pair for your product or AI agent, and share eligible Testnet results with a branded Geomacro card.</p></div>
  </section>
  ...
  <section class="panel">
    ...
    <div class="steps">
      ...
      <div class="step"><span class="n">03</span><b>Activate & test</b><div class="muted">250 Testnet USDC → 500 fixed credits</div></div>
    </div>
  </section>
  ...
  <section id="paymentPanel" class="panel" hidden>
    ...
    <div class="price">250 Testnet USDC</div>
    <div class="muted">500 fixed credits · 0.5 Testnet USDC per credit · 30 days · one quota per verified wallet · Testnet only · non-revenue.</div>
    ...
    <div class="notice">Activation verifies the supported chain, Testnet USDC contract, verified payer wallet, dedicated receiver, minimum 250 Testnet USDC amount and confirmed receipt. Replay never grants extra credits.</div>
  </section>
  ...
  <section id="developerPanel" class="panel" hidden>
    ...
    <p class="muted">Each Testnet developer credential is an API Key + API Secret pair. The secret is displayed once and only its hash is stored server-side.</p>
  ...
  <section id="feedbackPanel" class="panel" hidden>
    ...
    <h2>Run a real intelligence test, publish the result, then tell us what blocked you.</h2>
    <p class="muted">Open the Testnet Console, run an eligible metered intelligence request, create the professional Geomacro social card, then use “Share result on X”. After the test, send structured product feedback below. Feedback storage intentionally excludes your IP address and wallet address.</p>
    <div class="actions"><a href="/testnet-console">Open Testnet Console</a></div>
    <form id="testerFeedbackForm" class="form-grid">
      <div class="field"><label for="feedbackTesterType">You are testing as</label><select id="feedbackTesterType"><option value="builder">Builder</option><option value="agent_project">AI agent project</option><option value="institution">Institution / professional team</option><option value="researcher">Researcher</option><option value="other">Other</option></select></div>
      <div class="field"><label for="feedbackRating">Overall usefulness</label><select id="feedbackRating"><option value="5">5 · Very useful</option><option value="4" selected>4 · Useful</option><option value="3">3 · Mixed</option><option value="2">2 · Limited</option><option value="1">1 · Not useful</option></select></div>
      <div class="field"><label for="feedbackOutcome">Test outcome</label><select id="feedbackOutcome"><option value="worked">Worked</option><option value="partly_worked">Partly worked</option><option value="blocked">Blocked</option><option value="exploring">Still exploring</option></select></div>
      <div class="field"><label for="feedbackWouldIntegrate">Would you integrate this?</label><select id="feedbackWouldIntegrate"><option value="unsure">Not sure yet</option><option value="yes">Yes</option><option value="no">No</option></select></div>
      <div class="field full"><label for="feedbackMostValuable">Most valuable part</label><textarea id="feedbackMostValuable" maxlength="1000" placeholder="What was most useful?"></textarea></div>
      <div class="field full"><label for="feedbackFriction">Friction or confusion</label><textarea id="feedbackFriction" maxlength="2000" placeholder="What felt confusing, slow or broken?"></textarea></div>
      <div class="field full"><label for="feedbackMissingCapability">Missing capability</label><textarea id="feedbackMissingCapability" maxlength="1000" placeholder="What would make this useful in a real workflow?"></textarea></div>
      <div class="field full"><button type="submit">Send tester feedback</button></div>
    </form>
