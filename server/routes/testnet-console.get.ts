...
<style>
:root{color-scheme:dark;font-family:Inter,ui-sans-serif,system-ui,sans-serif;background:#080d16;color:#f4f2ea;--line:rgba(255,255,255,.08);--card:rgba(255,255,255,.035);--amber:#ff9d19;--muted:#979fab}*{box-sizing:border-box}body{margin:0;background:radial-gradient(900px 460px at 10% -10%,rgba(255,157,25,.09) 0,rgba(8,13,22,0) 60%),#080d16}main{max-width:980px;margin:auto;padding:36px 22px 80px}.nav{display:flex;justify-content:space-between;align-items:center;gap:16px;margin-bottom:48px}.brand img{display:block;height:40px;width:auto}.badge{font:12px ui-monospace,monospace;padding:7px 11px;border:1px solid var(--line);border-radius:999px;color:var(--muted);background:var(--card)}.language-select{width:auto;min-width:132px;padding:7px 28px 7px 10px;border-radius:999px;font:12px ui-monospace,monospace;color:#f4f2ea;background:#0d131d;border:1px solid var(--line)}.hero h1{font-size:42px;margin:10px 0 14px;letter-spacing:-.02em}.muted{color:var(--muted);line-height:1.55;font-size:14px}.panel{background:var(--card);border:1px solid var(--line);border-radius:16px;padding:20px;margin-top:22px}.eyebrow{font:12px ui-monospace,monospace;letter-spacing:.12em;color:var(--amber)}.form-grid{display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-top:14px}.field{display:flex;flex-direction:column;gap:6px}.field.full{grid-column:1/-1}label{font-size:12px;color:var(--muted)}input,select,textarea,button{width:100%;border:1px solid var(--line);background:rgba(255,255,255,.02);color:#f4f2ea;border-radius:10px;padding:11px 12px;font:14px inherit}textarea{min-height:88px;resize:vertical}button{cursor:pointer;font-weight:700;background:var(--amber);color:#0d1117;border-color:transparent}button:hover{filter:brightness(1.06)}.actions{display:flex;gap:10px;flex-wrap:wrap;margin-top:14px}.actions button{width:auto}.secondary{background:rgba(255,255,255,.04);color:#f4f2ea;border:1px solid var(--line)}.flow-steps{display:grid;grid-template-columns:repeat(5,1fr);gap:8px;margin:18px 0 4px}.flow-step{border:1px solid var(--line);border-radius:10px;padding:9px 8px;text-align:center;color:var(--muted);font:11px ui-monospace,monospace;background:rgba(255,255,255,.02)}.flow-step.active{border-color:var(--amber);color:#f4f2ea;background:rgba(255,157,25,.08)}.flow-step.done{border-color:rgba(120,220,150,.35);color:#b8e9c7}.notice{margin-top:14px;padding:12px 14px;border-radius:12px;background:rgba(255,255,255,.02);border:1px solid var(--line);color:var(--muted);font-size:13px;line-height:1.55}.okline{min-height:22px;margin-top:10px;color:var(--muted);font-size:13px}a{color:var(--amber)}@media(max-width:720px){.flow-steps{grid-template-columns:1fr 1fr}.form-grid{grid-template-columns:1fr}.field.full{grid-column:auto}.hero h1{font-size:34px}}
</style>
...
<body>
<main>
  <div class="nav"><div class="brand"><img src="/__l5e/assets-v1/9118c3e7-7750-4173-90cc-dd0445585e92/geomacro-logo.png" alt="Geomacro" height="40" /></div><div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;justify-content:flex-end"><select id="languageSelect" class="language-select" aria-label="Language"><option value="en">English</option><option value="es">Español</option><option value="fr">Français</option><option value="de">Deutsch</option><option value="pt">Português</option><option value="zh-CN">简体中文</option><option value="zh-TW">繁體中文</option><option value="ja">日本語</option><option value="ko">한국어</option><option value="hi">हिन्दी</option><option value="bn">বাংলা</option><option value="ar">العربية</option><option value="ru">Русский</option><option value="tr">Türkçe</option><option value="id">Bahasa Indonesia</option></select><div class="badge">TECHNICAL PROOF · PUBLIC TESTNET</div></div></div>
  <section class="hero">
  ...
  <section id="consoleAnchor" class="panel">
    <div class="eyebrow">LIVE x402 FLOW</div>
    <div class="flow-steps" aria-label="x402 payment flow"><div class="flow-step active">01 · API call</div><div class="flow-step">02 · 402 response</div><div class="flow-step">03 · Pay</div><div class="flow-step">04 · Retry</div><div class="flow-step">05 · Response</div></div>
    <div class="eyebrow" style="margin-top:22px">ACCOUNT CHECK</div>
    <p class="muted">If your wallet-verified Testnet access is active, the console appears below automatically. The 500 credits are a 30-day usage cap, not a prepaid balance.</p>
  </section>
</main>
</body>
</html>`;
