...
export default defineEventHandler(async (event) => {
  ...
  if (oauthError) return sendRedirect(event, "/testnet-access?oauth=x_declined", 302);
  if (!code || !state) {
    throw createError({ statusCode: 400, statusMessage: "X_OAUTH_CALLBACK_INVALID" });
  }
  ...
  try {
    ...
    return sendRedirect(event, "/testnet-access?oauth=x_connected", 302);
  } catch (error) {
    const codeName = error instanceof Error ? error.message : "X_OAUTH_CALLBACK_FAILED";
