...
import { settleTestnetApiCall } from "../../../src/lib/testnet-api-payment.server";

const MAX_BODY_BYTES = 8 * 1024;
const STRUCTURAL_RESPONSE_SCHEMA_VERSION = "geomacro-structural-intelligence-v1.0.0" as const;
...
function assertCapabilityMatchesSubject(
  ...
  if (
    (expectsCountry && subjectType !== "country") ||
    (!expectsCountry && subjectType !== "corridor")
  ) {
...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    // Read-only availability check happens before a payment is requested so a
    // buyer is never asked to pay for a subject that cannot currently be served.
    ...
    if (tier === "testnet_tester") {
      ...
      testnetPayment = settlement.payment;
    } else {
    ...
    try {
      await recordCommercialUsageEvent({
        ...
        credits_charged: usage.idempotent_replay
          ? 0
          : usage.credit_cost ?? GEOMACRO_CREDIT_COSTS[input.capability],
        ...
        execution_authorized: false,
        shareable: true,
        metadata: testnetPayment
          ? {
              testnet_payment_model: "pay_per_call",
              payment_event_id: testnetPayment.payment_event_id,
            }
          : {},
      });
