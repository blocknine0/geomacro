...
export async function fetchComtrade(countryIso3,{period="2025",cmdCode="TOTAL",flowCode="X",partnerCode="0",partner2Code="0"}={}) {
  ...
  const url=`https://comtradeapi.un.org/public/v1/preview/C/A/HS?reporterCode=${encodeURIComponent(countryIso3)}&period=${period}&cmdCode=${cmdCode}&flowCode=${flowCode}&partnerCode=${partnerCode}&partner2Code=${partner2Code}&motCode=0&maxRecords=500&customsCode=0&breakdownMode=classic&includeDesc=true&subscription-key=${encodeURIComponent(key)}`;
