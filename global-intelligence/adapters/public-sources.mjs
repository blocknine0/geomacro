...
export const publicSources={
  ...
  imf:async()=>{const data=await fetchJson("https://www.imf.org/external/datamapper/api/v2/NGDP_RPCH/USA");const values=data?.values?.NGDP_RPCH?.USA;if(!values||typeof values!=="object")throw new Error("IMF DataMapper response missing USA series");return{source_id:"imf_sdmx",observations:Object.keys(values).length};},
  oecd:async()=>{const body=await fetchText("https://sdmx.oecd.org/public/rest/dataflow/all?format=csvfilewithlabels");if(body.trim().split("\n").length<2)throw new Error("OECD dataflow response empty");return{source_id:"oecd_sdmx",observations:body.trim().split("\n").length-1};},
  bis:async()=>{const body=await fetchText("https://stats.bis.org/api/v2/dataflow/all/all/latest?format=csvfile");if(body.trim().split("\n").length<2)throw new Error("BIS dataflow response empty");return{source_id:"bis_statistics",observations:body.trim().split("\n").length-1};},
  eurostat:async()=>{const data=await fetchJson("https://ec.europa.eu/eurostat/api/dissemination/statistics/1.0/data/namq_10_gdp?geo=DE&na_item=B1GQ&unit=CLV10_EUR_HAB&sinceTimePeriod=2024");if(!data?.id?.length||data.value==null)throw new Error("Eurostat response missing observations");return{source_id:"eurostat",observations:Array.isArray(data.value)?data.value.length:Object.keys(data.value).length};},
  ...
  comtrade:async()=>{const data=await fetchJson("https://comtradeapi.un.org/public/v1/preview/C/A/HS?period=2024&reporterCode=840&cmdCode=TOTAL&flowCode=M&partnerCode=0&partner2Code=0&motCode=0&maxRecords=5");const rows=data?.data;if(!Array.isArray(rows)||!rows.length)throw new Error("UN Comtrade returned no rows");return{source_id:"un_comtrade",observations:rows.length};},
  faostat:async()=>{const body=await fetchText("https://fenixservices.fao.org/api/faostat/definitions/domain");if(body.length<20)throw new Error("FAOSTAT response too short");return{source_id:"faostat_global",observations:1};},
  ...
  bgs:async()=>{const data=await fetchJson("https://ogcapi.bgs.ac.uk/collections/wms_world_minerals_statistics/items?limit=1&f=json");if(!data||!Array.isArray(data.features))throw new Error("BGS response missing features");return{source_id:"bgs_world_minerals_statistics",observations:data.features.length};},
  ...
  copernicus_ems:async()=>{const data=await fetchJson("https://odp.dataspace.copernicus.eu/odata/v1/Workflows");if(!data)throw new Error("Copernicus ODP response empty");return{source_id:"copernicus_ems",observations:Array.isArray(data.value)?data.value.length:1};}
};
