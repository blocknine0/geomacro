...
export const publicSources={
  ...
  imf:async()=>{const data=await fetchJson("https://sdmxcentral.imf.org/sdmx/v2/dataflow/IMF/all/latest");if(!data||typeof data!=="object")throw new Error("IMF SDMX dataflow response missing");return{source_id:"imf_sdmx",observations:1};},
  ...
  comtrade:async()=>{const data=await fetchJson("https://comtradeapi.un.org/public/v1/preview/C/A/HS?flowCode=X&reporterCode=840&period=2024&partnerCode=0&cmdCode=TOTAL&maxRecords=5");const rows=data?.data;if(!Array.isArray(rows)||!rows.length)throw new Error("UN Comtrade returned no rows");return{source_id:"un_comtrade",observations:rows.length};},
