 ...
 *
 * Note on currency: this contract uses native `msg.value` for stakes, votes, and the
 * dispute fee rather than an ERC-20 transfer. On Arc, the native gas token IS USDC
 * (18 decimals), so "50 USDC" here means 50 * 10**6 of native value, matching Arc's
 * USDC-as-gas design. This is intentional, not a naming error.
 */
contract AgentArena {
    address public owner;
    address public treasury;

    uint256 public constant DISPUTE_FEE = 50 * 10**6;             // 50 USDC
    uint256 public constant DISPUTE_WINDOW = 24 * 60 * 60;        // 24 hours
    uint256 public constant MIN_VOLUME_FOR_DISPUTE = 500 * 10**6; // 500 USDC
    uint256 public constant MIN_VOTE_AMOUNT = 5 * 10**6;          // 5 USDC
    uint256 public constant PROTOCOL_FEE_BPS = 150;               // 1.5% (150 BPS)
    uint256 public constant TREASURY_DISPUTE_SHARE_BPS = 3000;    // 30%
    ...
    // Kept private to avoid stack-too-deep in downstream view functions.
